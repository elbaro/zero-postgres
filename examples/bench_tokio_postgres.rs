...
//!
//! Usage:
//!   DATABASE_URL=postgres://user:pass@localhost/test cargo run --example bench_tokio_postgres
...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for iteration in 0..10 {
        ...
        for (username, age, email, score, description) in rows.iter() {
            client
                .execute(&stmt, &[username, age, email, score, description])
                .await?;
        }
        ...
        client.execute("TRUNCATE TABLE test_bench", &[]).await?;
    }
