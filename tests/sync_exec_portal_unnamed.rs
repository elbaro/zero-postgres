...
fn exec_portal_with_raw_sql() {
    ...
    let total: i32 = conn
        .exec_portal("SELECT generate_series(1, 5) as n", (), |portal| {
            let mut handler = CollectHandler::new();
            portal.exec(0, &mut handler)?;
            let rows: Vec<(i32,)> = handler.into_rows();
            Ok(rows.iter().map(|(n,)| n).sum())
...
fn exec_portal_with_raw_sql_and_params() {
    ...
    let total: i32 = conn
        .exec_portal("SELECT generate_series(1, $1) as n", (5i32,), |portal| {
            let mut handler = CollectHandler::new();
            portal.exec(0, &mut handler)?;
            let rows: Vec<(i32,)> = handler.into_rows();
            Ok(rows.iter().map(|(n,)| n).sum())
...
fn exec_portal_raw_sql_batched() {
    ...
    let mut all_rows: Vec<i32> = Vec::new();
    let batch_count: i32 = conn
        .exec_portal("SELECT generate_series(1, 10) as n", (), |portal| {
            let mut batches = 0;
            loop {
                let mut handler = CollectHandler::new();
                let has_more = portal.exec(3, &mut handler)?; // fetch 3 at a time
